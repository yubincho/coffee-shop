...
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.2.10-green)
![Redis](https://img.shields.io/badge/Redis-Redisson-red)
![Elasticsearch](https://img.shields.io/badge/Elasticsearch-nori-yellow)
...
- Google OAuth2 로그인
- 상품 관리 (CRUD, 커서 기반 페이징)
- 상품 검색 (Elasticsearch + nori 형태소 분석기, 약 10만 건 색인)
- 장바구니 담기 / 수량 변경
- 커피 주문 및 주문 확정 (결제는 상태 전이 방식의 가결제)
- 결제 상태 조회 캐싱 (Redis 우선 조회 + DB fallback, 캐시 무효화)
...
애플리케이션 실행 전에 Docker Compose로 필요한 컨테이너를 먼저 띄운다.
...
| 컨테이너 | 용도 |
| --- | --- |
| redis | 캐시 · 분산 락 (Redisson) |
| elasticsearch | 상품 검색 (nori 형태소 분석기) |
| broker | Kafka 브로커 (사용자 활동 이벤트) |
| zookeeper | Kafka 클러스터 코디네이션 |
| influxdb | k6 부하 테스트 결과 저장 |
| grafana | 부하 테스트 결과 시각화 |

### 3. 데이터베이스 설정

- MySQL은 로컬에 별도 설치되어 있어야 한다.
- MySQL 실행 후 `application.yml`에 DB 접속 정보를 설정한다.

### 4. 애플리케이션 실행

인프라 컨테이너가 모두 뜬 것을 확인한 뒤 스프링 서버를 실행한다.
```bash
./gradlew bootRun
```
...
- **부하 테스트 (조회 API)**: 상품 100,003건 · 300 VU · 5분 조건에서 k6로 측정했다.
  ...
  | 캐시 적용 (실제 패턴: 랜덤 커서 + 키워드) | 약 3.87s | 약 86 | 0% | - |
   ...
   | 구간 | p95 | |
   | --- | --- | --- |
   | 캐시 전 | 2.79s | ███████████████████ (약 2,790ms) |
   | 캐시 적용 | 25.89ms | ▏ (약 108배 개선) |
...
### 7. 상품 검색 Elasticsearch 전환 (LIKE 풀스캔 제거)

**문제**: 상품 검색의 `LIKE '%keyword%'`는 선행 와일드카드 때문에 B-tree 인덱스를 타지 못하고 풀스캔이 발생했다. 부하 테스트에서 랜덤 커서·키워드가 섞인 실제 패턴의 p95가 약 3.87s까지 치솟았고, 캐시로도 인덱스로도 해결되지 않는 병목이었다.

**해결**: 검색을 Elasticsearch로 분리했다. nori(한글 형태소) 분석기를 적용한 커스텀 이미지를 Docker로 빌드하고, `ProductDocument`(name·description을 nori로 색인)와 `ProductSearchRepository`를 추가했다. 검색은 Elasticsearch에서 상품 ID만 조회한 뒤, 그 ID로 MySQL에서 이미지 fetch join으로 실제 데이터를 가져오는 방식으로 구현해 기존 커서 페이징 구조를 그대로 유지했다. 커서 페이징을 위해 `productId`를 별도 Long 필드로 색인했다(`@Id`는 `_id` 문자열로 매핑되어 범위·정렬에 부적합). 기존 MySQL 상품 약 10만 건을 bulk로 초기 색인했다.

![img_5.png](img_5.png)

<br>

### 8. 결제 상태 조회 Redis 캐싱 및 DB Fallback

**문제**: 결제 상태 체크는 조회 빈도가 높은데(결제 진행 중 폴링·여러 화면 조회) 매번 DB를 직접 조회해, 트래픽이 몰리면 DB QPS와 CPU가 함께 치솟는 구조였다.
(현재 결제는 실제 PG 연동 없이 상태만 전이하는 가결제 단계다.)

**해결**: 상태 조회 경로에 "Redis 우선 조회 + DB fallback" 구조를 적용했다. Redis에 상태가 있으면 즉시 응답하고, miss이거나 Redis 장애 시 MySQL을 조회한 뒤 결과를 Redis에 재저장한다. 상태 조회 전용 경량 객체(`OrderStatusCache`)를 두어 무거운 DTO의 연관관계·지연 로딩 문제를 피했고, 결제 완료 시 캐시를 무효화(delete)해 정합성을 유지했다. Redis 접근은 모두 try-catch로 감싸 Redis가 죽어도 서비스가 정상 동작하도록 했으며, 캐싱 전용 Lettuce 연결에만 100ms command timeout을 적용해(Redisson 락과의 충돌 방지) 느린 Redis에도 빠르게 fallback되도록 했다.
![img_1.png](img_1.png)

**부하 테스트 (조회 집중 시나리오, 100 RPS · 3분)**:

| 지표 | 캐싱 전 | 캐싱 후 |
| --- | --- | --- |
| DB QPS | 약 598 | 약 2 |
| p95 응답 시간 | 50.95ms | 8.26ms |

조회가 특정 주문에 집중되는 실제 폴링 패턴에서 DB QPS가 약 99.7% 감소했다. 다만 조회 대상이 수천 건으로 넓게 분산되면 캐시 히트율이 낮아 Redis 왕복이 오히려 오버헤드가 되는 것도 함께 확인해, 캐싱이 효과적인 조건(히트율이 높은 조회 집중 상황)을 명확히 했다.
...
- **검색 품질 개선**: 현재 name·description만 검색 대상이며, brand(Keyword 매핑)는 부분 일치가 안 된다. 향후 brand를 nori Text로 변경하고 필드별 가중치(boosting)를 조정해 검색 정확도를 높일 계획이다.
- **실제 결제 연동**: 현재는 상태 전이만 처리하는 가결제 수준(`/orders/done`에서 CONFIRMED로 변경)이다. 향후 아임포트 결제 검증(imp_uid로 실제 결제·금액 확인)까지 연동해 결제 흐름을 완성할 계획이다.
